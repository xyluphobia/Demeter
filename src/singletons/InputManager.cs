...
public partial class InputManager : Node
{
  ...
  [Signal] public delegate void DraggingEventHandler(Vector2 delta);
  ...
  private void HandleScreenTouch(InputEventScreenTouch touch) {
    if (touch.Pressed) {
      if (touch.Index == 0) {
        ...
        _isDragging = false;
        EmitSignal(SignalName.Dragging, Vector2.Zero);
      }
  ...
  private void HandleScreenDrag(InputEventScreenDrag drag) {
    if (drag.Index == 0) {
        ...
        if (_touch0StartPosition.DistanceTo(drag.Position) > TapThreshold) {
          _isDragging = true;
          EmitSignal(SignalName.Dragging, drag.Relative);
        }
    }
  ...
  private void HandleMouse(InputEventMouseButton mouseButton) {
    if (mouseButton.Pressed) {
      if (mouseButton.ButtonIndex == MouseButton.WheelUp) {
        EmitSignal(SignalName.PinchChanged, 100f);
      }
